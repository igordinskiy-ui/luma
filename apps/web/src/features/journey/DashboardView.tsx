...
export function DashboardView({ dashboard, refresh, initialScreen = 'home', updatePlan = api.plan }: DashboardViewProps) {
  ...
  return <main className="path-app-shell">
    <header className="path-app-header"><div className="path-wordmark"><span>П</span><b>Последняя пачка</b></div><button type="button" aria-label="Открыть настройки" onClick={() => changeScreen('settings')}>⚙</button></header>
    ...
    <section className={`path-guidance risk-${dashboard.risk}`} aria-live="polite"><span>{notice ? 'Обновление' : 'Следующий шаг'}</span><p>{notice || dashboard.intervention}</p>{dashboard.recent_triggers.length > 0 && <small>Недавно отмечалось: {dashboard.recent_triggers.map(triggerLabel).join(' · ')}</small>}</section>
