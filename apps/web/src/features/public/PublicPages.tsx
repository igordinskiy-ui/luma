...
import { beginOidcLogin } from '../../api';
...
export function GuidePage({ guide }: { guide: GuideName }) {
  ...
  useEffect(() => setMetadata(`${content.title} — Последняя пачка`, content.intro), [content]);
  return <main className="path-page path-guide"><header className="path-public-header"><a className="path-wordmark" href="/"><span>П</span><b>Последняя пачка</b></a><a href="/">О продукте</a></header><article><span className="path-kicker">{content.kicker}</span><h1>{content.title}</h1><p className="path-guide-intro">{content.intro}</p><ol>{content.steps.map((step, index) => <li key={step}><i>{index + 1}</i><span>{step}</span></li>)}</ol><aside>Это немедицинский помощник. При плохом самочувствии или экстренной ситуации обратись к врачу или местной службе помощи.</aside><a className="path-button primary" href="/app">Открыть помощника <span>→</span></a></article></main>;
}

export function Landing() {
  useEffect(() => setMetadata('Последняя пачка', 'Немедицинский помощник на пути от последней пачки к жизни без сигарет — без стыда и обещаний невозможного.'), []);
  return <main className="path-page path-landing">
    <header className="path-public-header"><a className="path-wordmark" href="/"><span>П</span><b>Последняя пачка</b></a>{import.meta.env.DEV && <a href="/dev/designs">Интерактивный интерфейс</a>}</header>
    <section className="path-landing-hero">
      <div className="path-landing-copy"><span className="path-kicker">Немедицинский помощник</span><h1>Не идеальная жизнь.<br /><em>Следующий честный шаг.</em></h1><p>Поддержка на пути от последней пачки к жизни без сигарет — без стыда, давления и обещаний невозможного.</p><div className="path-landing-actions"><a className="path-button primary login" href="/api/v1/auth/oidc/start" onClick={event => { event.preventDefault(); beginOidcLogin(); }}>Войти через Telegram <span>→</span></a><a className="path-button ghost" href="#how">Как это работает</a></div><small>Работает как Telegram Mini App и обычное PWA.</small></div>
