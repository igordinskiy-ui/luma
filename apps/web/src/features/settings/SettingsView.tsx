...
export function SettingsView({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => { Promise.all([api.preferences(), api.notificationStatus(), api.quitPlan()]).then(([nextPreferences, status, nextPlan]) => { setPreferences(nextPreferences); setChannel(status); setPlan(nextPlan); }).catch(() => setMessage('Не удалось загрузить часть настроек. Можно повторить позже.')); }, []);
  ...
  const eraseAccount = async () => {
    if (deletePhrase !== 'УДАЛИТЬ') return;
    try { await api.erase(); await clearDeviceSession(); location.assign('/'); }
    catch { setMessage('Не удалось удалить аккаунт. Данные не изменены — попробуй ещё раз.'); setDeleteOpen(false); }
  };
  ...
  return <main className="path-app-shell path-settings-page">
    ...
    <section className="path-settings-card"><header><div><span>Напоминания</span><h2>Поддерживающие сообщения</h2></div><label className="path-switch"><span className="sr-only">Разрешить поддерживающие сообщения</span><input type="checkbox" checked={preferences.enabled} onChange={event => setPreferences({ ...preferences, enabled: event.target.checked })} /><i /></label></header><p className="path-setting-explainer">Если включить сообщения, мы напомним о выбранном шаге — без заметок, причин и других личных подробностей. Сначала сохрани расписание, затем выбери канал.</p><label>Максимум сообщений в день<select value={preferences.max_daily} onChange={event => setPreferences({ ...preferences, max_daily: Number(event.target.value) })}>{[0, 1, 2, 3, 4, 5, 6].map(value => <option key={value} value={value}>{value === 0 ? 'Не отправлять' : value}</option>)}</select></label><div className="path-field-grid"><label>Не беспокоить с<input type="number" min="0" max="23" value={preferences.quiet_start} onChange={event => setPreferences({ ...preferences, quiet_start: Number(event.target.value) })} /></label><label>до<input type="number" min="0" max="23" value={preferences.quiet_end} onChange={event => setPreferences({ ...preferences, quiet_end: Number(event.target.value) })} /></label></div><button type="button" className="path-button primary" onClick={saveNotificationPreferences}>Сохранить расписание <span>→</span></button><div className="path-channel-card"><div><span>Telegram</span><b>{channel?.telegram === 'available' ? 'Доступен' : 'Не настроен'}</b></div><div><span>Web push</span><b>{channel?.web_push === 'subscribed' ? 'Подключён' : 'Не подключён'}</b></div></div><div className="path-notification-actions"><button type="button" className="path-button ghost" onClick={enablePush}>Подключить web push</button>{channel?.web_push === 'subscribed' && <button type="button" className="path-button ghost" onClick={disablePush}>Отключить web push</button>}<button type="button" className="path-button quiet" disabled={!preferences.enabled} onClick={sendTest}>Отправить тест</button></div></section>
    ...
    <section className="path-danger-zone"><h2>Удаление аккаунта</h2><p>Активные данные, очереди и подписки будут удалены. Ротация резервных копий описана в политике хранения.</p><button type="button" onClick={() => { setDeletePhrase(''); setDeleteOpen(true); }}>Удалить аккаунт</button></section>
    <PathDialog open={deleteOpen} onClose={() => setDeleteOpen(false)} labelledBy="delete-title"><header><div><span className="path-kicker">Необратимое действие</span><h2 id="delete-title">Удалить весь путь?</h2></div><button type="button" aria-label="Закрыть" onClick={() => setDeleteOpen(false)}>×</button></header><p>Экспортируй данные заранее, если хочешь сохранить копию. Для подтверждения введи <b>УДАЛИТЬ</b>.</p><label>Подтверждение<input data-autofocus autoComplete="off" value={deletePhrase} onChange={event => setDeletePhrase(event.target.value)} /></label><button className="path-button danger" disabled={deletePhrase !== 'УДАЛИТЬ'} type="button" onClick={eraseAccount}>Удалить аккаунт безвозвратно</button></PathDialog>
