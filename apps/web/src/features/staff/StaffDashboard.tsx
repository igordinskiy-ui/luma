...
export function StaffDashboard() {
  ...
  const percentOrEmpty = (rate: number, denominator: number, empty = 'нет данных') => denominator ? `${Math.round(rate * 100)}%` : empty;
  const cohort = (metric: { eligible: number; retained: number; rate: number }) => metric.eligible ? `${metric.retained}/${metric.eligible} (${Math.round(metric.rate * 100)}%)` : 'нет полного окна';
  const deliveryAttempts = (overview?.deliveries_last_24h.sent || 0) + (overview?.deliveries_last_24h.failed || 0);
  ...
  return <main className="path-app-shell path-staff-page">
    ...
    {overview && <>
      ...
      <p className="path-metric-note">D1, D7 и D14 — действие в соответствующие 24 часа пути. В знаменатель входят только полностью завершённые окна; поздний возврат не меняет прошлую retention. Delivery failure считается только по завершённым попыткам: sent + failed, без очереди. Crash-free: <b>{overview.client_health.sessions ? `${(overview.client_health.crash_free_rate * 100).toFixed(2)}%` : 'нет данных'}</b> ({overview.client_health.crashed} из {overview.client_health.sessions} сессий).</p>
      <section className="card"><h2>Операционный статус</h2><p>Контент: <b>{overview.content_review_status}</b> · версия <code>{overview.content_version}</code> · отпечаток <code>{overview.content_digest.slice(0, 12)}</code> · режим общих подсказок: <b>{overview.risk_engine_version}</b></p><p>Activation: <b>{overview.activation.onboarded}</b> ({percentOrEmpty(overview.activation.rate, overview.users_total)}) · mute rate: <b>{percentOrEmpty(overview.notification_health.mute_rate, overview.notification_health.preferences_total)}</b></p><p>Первое действие ≤24 ч: <b>{overview.funnel.first_action_24h}/{overview.funnel.onboarded}</b> ({percentOrEmpty(overview.funnel.first_action_rate, overview.funnel.onboarded)})</p><p>D1: {cohort(overview.retention.d1)} · D7: {cohort(overview.retention.d7)} · D14: {cohort(overview.retention.d14)}</p><p>Delivery failures за 24 часа: <b>{overview.notification_health.delivery_failures_last_24h}</b> ({percentOrEmpty(overview.notification_health.delivery_failure_rate, deliveryAttempts, 'нет завершённых попыток')})</p><p>Планы: {Object.entries(overview.plans_by_phase).map(([phase, count]) => `${phase}: ${count}`).join(' · ') || 'нет'}</p><p>Outbox: {Object.entries(overview.outbox_by_status).map(([status, count]) => `${status}: ${count}`).join(' · ') || 'нет'}</p></section>
