...
export function JournalView({ onBack, onSupport, loadJournal = api.journal, updateEvent = api.updateEvent, deleteEvent = api.deleteEvent }: Props) {
  ...
  return <main className="path-app-shell path-journal-page">
    ...
    <p className="path-edit-policy">Время показано по часовому поясу этого устройства. Недавнюю поведенческую запись можно исправить в течение 15 минут; после этого она остаётся частью хронологии.</p>
