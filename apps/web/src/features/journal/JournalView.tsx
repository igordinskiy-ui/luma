...
import { api, JournalItem, JournalResponse } from '../../api';
import { PathDialog } from '../../ui/PathDialog';
import './journal.css';

const triggers = [
  ['', 'Все триггеры'], ['stress', 'Стресс'], ['coffee', 'Кофе'], ['after_meal', 'После еды'],
...
const techniqueLabels = { breathing: 'медленный выдох', water: 'стакан воды', walk: 'короткая прогулка' } as const;
...
export function JournalView({ onBack, onSupport, loadJournal = api.journal, updateEvent = api.updateEvent, deleteEvent = api.deleteEvent }: Props) {
  ...
  return <main className="path-app-shell path-journal-page">
    ...
    {Object.entries(groups).map(([date, entries]) => <section className="path-event-group" key={date}><header><h2>{date}</h2><span>{entries.length}</span></header><ol className="path-event-list">{entries.map(item => { const editable = item.source === 'event' && Boolean(item.editable_until) && new Date(item.editable_until!).getTime() > Date.now(); return <li key={item.id}><article><i className={`kind-${item.type}`}>{item.type === 'coping' ? '✓' : item.type === 'craving' ? '≈' : item.type === 'relapse' ? '↻' : '•'}</i><div><b>{itemLabels[item.type]}</b><span>{triggerLabel(item.trigger)}{item.intensity_before ? ` · ${item.intensity_before} из ${item.source === 'coping' ? 10 : 5}` : ''}</span>{item.type === 'coping' && <p>{item.technique ? techniqueLabels[item.technique] : 'Способ не выбран'}{item.intensity_after ? ` · после ${item.intensity_after} из 10` : ` · ${item.status === 'paused' ? 'на паузе' : item.status === 'abandoned' ? 'завершено раньше' : item.status}`}</p>}{item.note && <p>{item.note}</p>}</div><div className="path-event-meta"><time dateTime={item.created_at}>{new Date(item.created_at).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</time>{editable && <button type="button" onClick={() => beginEdit(item)}>Исправить</button>}</div></article></li>; })}</ol></section>)}
