...
export function App({ initialScreen = 'home' }: { initialScreen?: AppScreen }) {
  const [state, setState] = useState<'loading' | 'landing' | 'auth' | 'consent' | 'onboarding' | 'dashboard' | 'offline' | 'rate' | 'maintenance' | 'error'>('loading');
  ...
  const refresh = async () => {
    ...
    try {
      ...
      else if (error instanceof ApiError && error.status === 503) setState('maintenance');
      else setState('error');
    }
  };
