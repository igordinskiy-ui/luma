...
describe('identity-bound offline queue', () => {
  ...
  it('replays a coping lifecycle sequentially after reconnecting', async () => {
    const create = { client_session_id: 'coping-offline-1', source: 'offline' as const, trigger: 'coffee' as const, intensity_before: 7 };
    enqueueCopingStart('user-a', create);
    enqueueCopingPatch('user-a', create.client_session_id, { technique: 'water' });
    enqueueCopingPatch('user-a', create.client_session_id, { status: 'completed', intensity_after: 3, outcome: 'helped' });
    ...
    expect(patch.mock.calls).toEqual([[42, { technique: 'water' }], [42, { status: 'completed', intensity_after: 3, outcome: 'helped' }]]);
    expect(localStorage.getItem('kurilka-coping-queue:user-a')).toBe('[]');
  });

  it('coalesces concurrent reconnects for the same user', async () => {
    const create = { client_session_id: 'coping-single-flight', source: 'offline' as const, trigger: 'coffee' as const, intensity_before: 7 };
    enqueueCopingStart('user-a', create);
